...
export default function Form({ resource, title, fields, record, options = {} }: any) {
    ...
    return <ResourceForm resource={resource} title={title} fields={fields} record={record} options={options}/>;
}

function ResourceForm({ resource, title, fields, record, options }: any) {
    ...
    const submit = (event: FormEvent) => {
        ...
        if (record) {
            ...
            form.post('/admin/' + resource + '/' + record.id, { forceFormData: true });
...
function Gallery({ record, data, set }: any) {
    ...
    const uploads = data.new_images as File[];
