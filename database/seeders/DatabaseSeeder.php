...
class DatabaseSeeder extends Seeder {
    public function run(): void {
        $profile=BusinessProfile::create(['name'=>'Technoout','legal_name'=>'Technoout Technology Consultants','tagline'=>'Engineered access. Intelligent security.','description'=>'Future-ready automation, access control, physical security and industrial entry solutions for Pakistan.','currency'=>'PKR','quote_prefix'=>'QTN','invoice_prefix'=>'INV','footer_text'=>'Automation, security and industrial access systems—engineered for reliable performance.']);
        $lahore=OfficeLocation::create(['name'=>'Lahore Head Office','address_line_1'=>'Ground Floor, Paragon House, 1-E/1, Shamshad Haider Road','city'=>'Lahore','region'=>'Punjab','postal_code'=>'54600','hours'=>'Monday–Saturday, 9:00 AM–5:00 PM','is_primary'=>true]);
        $islamabad=OfficeLocation::create(['name'=>'Islamabad Office','address_line_1'=>'Office No. 13-A, Second Floor, Rehmat Centre, I-8 Markaz','city'=>'Islamabad','region'=>'Islamabad Capital Territory','hours'=>'Monday–Saturday, 9:00 AM–5:00 PM']);
        foreach([[$lahore,'phone','Lahore phone','+92-42-35762213'],[$lahore,'mobile','Sales mobile','0340-7860901'],[$islamabad,'phone','Islamabad phone','+92-51-4938259'],[null,'email','General enquiries','info@technoout.pk']] as $c) ContactChannel::create(['office_location_id'=>$c[0]?->id,'type'=>$c[1],'label'=>$c[2],'value'=>$c[3],'is_primary'=>$c[1]==='email']);
        ...
        $map=[];foreach($cats as $i=>$name)$map[$name]=Category::create(['name'=>$name,'slug'=>Str::slug($name),'description'=>'Professional '.$name.' engineered, installed and supported across Pakistan.','sort_order'=>$i+1]);
        $products=[
        ...
        foreach($products as $i=>$p)Product::create(['name'=>$p[0],'slug'=>Str::slug($p[0]),'sku'=>'TO-'.str_pad((string)($i+1),3,'0',STR_PAD_LEFT),'category_id'=>$map[$p[1]]->id,'summary'=>$p[2],'description'=>$p[2].' Technoout provides survey, specification, installation, commissioning and after-sales support.','specifications'=>['Service'=>'Supply, installation and support','Coverage'=>'Pakistan','Warranty'=>'Project specific'],'price_mode'=>'quote','is_featured'=>$i<6,'is_published'=>true]);
        $pages=[
            ['page','About Technoout','about-us','A company constantly moving forward','Technoout delivers modern automatic entry, physical security, access control and industrial systems. Innovation, reliability and complete after-sales support guide every project.'],
            ...
            ['news','Latest News','latest-news','Insights from the field','Project updates, product developments and practical guidance from the Technoout team.'],
        ...
        $admin=User::create(['name'=>'Technoout Administrator','email'=>env('ADMIN_EMAIL','admin@technoout.pk'),'password'=>Hash::make(env('ADMIN_PASSWORD','ChangeMe123!')),'email_verified_at'=>now(),'is_active'=>true]);$admin->assignRole('Super Admin');
    }
}
